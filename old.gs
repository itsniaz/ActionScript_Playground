...
function onInstall(e) {
  ...
  function uploadSplittedTabsToDrive() {
    ...
    // Step 1: Prompt the user for the Google Drive folder URL
    ...
    // Step 2: Get the folder by ID
    var folder;
    try {
      folder = DriveApp.getFolderById(folderId);
    } catch (e) {
      ui.alert("Could not access the specified folder. Please check the URL and try again.");
      return;
    }
    ...
    // Step 3: Filter only "Split_X" tabs
    var splitSheets = sheets.filter(sheet => /^Split_\d+$/.test(sheet.getName()));
    ...
    // Step 4: Convert each Split_X sheet into a CSV file and upload
    splitSheets.forEach(sheet => {
    ...
    // Step 5: Alert the user with the folder link
